############################################################
## Balanced Nested Fixed-Effect Design Simulation
...
b  <- 4          # number of clusters nested within each treatment
...
############################################################
## 4. Generate Balanced Nested Fixed-Effect Data
############################################################

generate_nested_fixed_data <- function(a, b, n0, alpha, beta_mat, sigma2) {
  ...
  for (i in 1:a) {
    for (j in 1:b) {
      ...
      y_ijk <- alpha[i] + beta_mat[i, j] +
        rnorm(n0, mean = 0, sd = sqrt(sigma2))
...
############################################################
## 8. Estimate Fixed Nested Effect Contribution Under Blinding
############################################################

estimate_fixed_nested_component_blind <- function(dat) {
  ...
  beta_component_hat <- (MS_cluster - MSE) / n0_star
...
reestimate_sample_size <- function(
  ...
  sigmaA2_hat <- sigma2_int + nbar_B_star * beta_component_hat
...
one_simulation <- function(theta, beta_scale) {
  ...
  dat_full <- generate_nested_fixed_data(
    a = a,
    b = b,
    n0 = n0,
    alpha = alpha_plan,
  ...
  beta_component_hat <- estimate_fixed_nested_component_blind(dat_int)
...
############################################################
## 13. Summary of Re-estimated Sample Size
############################################################
...
############################################################
## 14. Convert Summary to Clean Data Frame
############################################################
...
write.csv(
  all_results,
  "balanced_nested_fixed_effect_full_simulation_results.csv",
  row.names = FALSE
)

write.csv(
  variance_summary,
  "balanced_nested_fixed_effect_variance_summary.csv",
  row.names = FALSE
)

write.csv(
  clean_sample_size_summary,
  "balanced_nested_fixed_effect_sample_size_summary.csv",
  row.names = FALSE
)
