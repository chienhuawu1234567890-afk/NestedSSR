############################################################
## Unbalanced Nested Design Simulation
## Example: n2 = 2 n1 and n3 = 3 n1
############################################################
...
b <- 4                 # clusters per treatment

n1 <- 8                # subjects per cluster in treatment 1
n_vec <- c(n1, 2*n1, 3*n1)
...
n_i_plan <- b * n_vec
w_i_plan <- n_i_plan / sum(n_i_plan)

delta2_plan <- sum(w_i_plan * alpha_plan^2)

############################################################
## 3. Generate Unbalanced Nested Data
############################################################

generate_nested_data_unbalanced <- function(a, b, n_vec,
                                            alpha, sigma2, sigmaB2) {
...
############################################################
## 7. Estimate sigma_B^2 Under Unbalanced Cluster Sizes
############################################################

estimate_sigmaB2_blind_unbalanced <- function(dat) {
  ...
  grand_mean <- weighted.mean(cluster_means, w = n_j)
  ...
  sigmaB2_hat <- (MS_cluster - MSE) / n_eff
...
reestimate_sample_size_unbalanced <- function(
    sigma2_int,
    sigmaB2_hat,
    delta2_plan,
    a,
  ...
  sigmaA2_hat <- sigma2_int + nbar_B * sigmaB2_hat
  ...
  N_reest <- ifelse(length(ok) == 0, NA, candidate_N[min(ok)])
...
one_simulation <- function(theta, sigmaB2_true) {
  ...
  dat_full <- generate_nested_data_unbalanced(
    a = a,
    b = b,
    n_vec = n_vec,
    alpha = alpha_plan,
  ...
  sigmaB2_hat <- estimate_sigmaB2_blind_unbalanced(dat_int)
  
  ssr_blind <- reestimate_sample_size_unbalanced(
  ...
  ssr_unblind <- reestimate_sample_size_unbalanced(
  ...
  ssr_adjusted <- reestimate_sample_size_unbalanced(
...
for (theta in theta_values) {
  for (sigmaB2 in sigmaB2_values) {
    ...
    temp_results <- do.call(
      ...
      replicate(
        nsim,
        one_simulation(theta = theta,
                       sigmaB2_true = sigmaB2),
        simplify = FALSE
      )
    )
...
write.csv(
  all_results,
  "unbalanced_nested_full_simulation_results.csv",
  row.names = FALSE
)

write.csv(
  variance_summary,
  "unbalanced_nested_variance_summary.csv",
  row.names = FALSE
)

write.csv(
  clean_sample_size_summary,
  "unbalanced_nested_sample_size_summary.csv",
  row.names = FALSE
)
